...
coverImage: https://blog.999622.xyz/images/linear-algebra-6/banner.jpeg
math: true
...
![线性变换应用于不同的向量](https://blog.999622.xyz/images/linear-algebra-6/eigenvector.png)
...
![两个一维特征向量张成两个一维特征空间](https://blog.999622.xyz/images/linear-algebra-6/eigenspace.png)

由于 $\mathbf{A}\mathbf{x}=\lambda\mathbf{x}$，因而线性变换 $\mathbf{A}$ 对于`特征空间`只起到「扩张」的作用（扩张后还是同样的`特征空间`）。

![特征空间的扩张](https://blog.999622.xyz/images/linear-algebra-6/span.png)
