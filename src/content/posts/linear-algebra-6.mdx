...
coverImage: https://blog.mengqi.life/images/linear-algebra-6/banner.jpeg
math: true
...
[上一章](/posts/linear-algebra-5) 最后，我们引入了`马尔可夫链`。`马尔可夫链`简单来说就是一个个状态组成的链，其中每个状态只于前一个状态有关。然而，除了这个简单定义之外，`马尔可夫链`还有一个有趣的性质：`平稳分布`。要解释`平稳分布`是什么，我们先从一个例子讲起。
...
假设每年的选举结果只和上一年的结果有关，即选举向量构成的序列满足马尔可夫性质，是一个`马尔可夫链`。那么，像 [上一章](/posts/linear-algebra-5) 那个人口迁移的例子一样，我们可以用一个`状态迁移矩阵`来描述每年选举结果的变化情况。
...
![线性变换应用于不同的向量](https://blog.mengqi.life/images/linear-algebra-6/eigenvector.png)
...
![两个一维特征向量张成两个一维特征空间](https://blog.mengqi.life/images/linear-algebra-6/eigenspace.png)

由于 $\mathbf{A}\mathbf{x}=\lambda\mathbf{x}$，因而线性变换 $\mathbf{A}$ 对于`特征空间`只起到「扩张」的作用（扩张后还是同样的`特征空间`）。

![特征空间的扩张](https://blog.mengqi.life/images/linear-algebra-6/span.png)
...
回顾 [上一章](/posts/linear-algebra-5) 那个关于城市人口迁移的研究，那个例子我们引入了`马尔可夫链`这个概念，而从这章我们知道`马尔可夫链`有个`平稳分布`的性质，那么[上一章](/posts/linear-algebra-5)那个人口迁移的例子最终也一定会达到某种稳定状态，即城乡人口比例保持不变。

[上一章](/posts/linear-algebra-5)中，我们已经得出：
$$
\mathbf{x}_0 = 
...
## 参考文献：
- [线性代数及其应用：第3版/（美）莱（Lay, D.C.）著；沈复兴等译. ——北京：人民邮电出版社，2007.7](https://book.douban.com/subject/30310517/)
