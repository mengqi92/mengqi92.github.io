...
tags:
  ...
  - Gabor 特征
category: 图像处理
description: Gabor 特征是一种可以用来描述图像纹理信息的特征，Gabor 滤波器的频率和方向与人类的视觉系统类似，特别适合于纹理表示与判别。 Gabor 特征主要依靠 Gabor 核在频率域上对信号进行加窗，从而能描述信号的局部频率信息。 说到 Gabor 核，不能不提到傅里叶变换。正是靠傅里叶变换，我们才能将信号转换到频率域，才能让Gabor核在频率域去加窗。而在原本的空间域中，一个 Gabor 核实际上就是一个高斯核与正弦波调制的结果，可以看做是高斯核应用在了正弦波的频域部分。 上面说的还是比较笼统，下面我们一步一步介绍Gabor核是怎么对信号「加窗」的。
coverImage: https://blog.mengqi.life/images/gabor/banner.jpeg
math: true
createdDate: 2015-10-11 17:02:00
modifiedDate: 2015-10-11 17:02:00
---
...
关于傅里叶变换，韩昊同学总结过一个[很直观的解释](https://zhuanlan.zhihu.com/wille/19763358)。我这里就不赘述了。
...
其中，f 为输入信号，$\xi$ 表示分解得到的各个波的频率，$\hat{f}(f, \xi)$ 为变换后的信号。公式中的 $e^{-i2\pi x \xi}$ 表示一个复数波，关于复数波的解释可以看我[之前的一篇文章](/posts/complex)
...
![二维高斯](https://blog.mengqi.life/images/gabor/2d-gaussian.png)
...
![Gabor 核频率域图示](https://blog.mengqi.life/images/gabor/gabor-filter-frequency.png)
...
![Gabor 核空间域图示](https://blog.mengqi.life/images/gabor/gabor-filter-spatial.png)
...
通过上面的分析，我们知道了，一个Gabor核能获取到图像某个频率邻域的响应情况，这个响应结果可以看做是图像的一个特征。那么，我们如果用多个不同频率的Gabor核去获取图像在不同频率邻域的响应情况，最后就能形成图像在各个频率段的特征，这个特征就可以描述图像的频率信息了
 ...
 ![ 一系列 Gabor 核](https://blog.mengqi.life/images/gabor/gabor-filter-banks.png)
...
2. [韩昊同学对傅里叶变换的直观解释](https://zhuanlan.zhihu.com/wille/19763358)
3. [中文维基百科 / 卷积定理](https://zh.wikipedia.org/wiki/卷积定理)
4. [英文维基百科 / Gabor_filter](https://en.wikipedia.org/Gabor_filter)
